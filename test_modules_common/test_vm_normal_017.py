...
class test_vm_normal_017(common.sensemediaTestBase):
    ids_2 = []

    def __init__(self):
        super(test_vm_normal_017, self).__init__("test_vm_normal_017")
        ...
        self.file = ""
        # TODO
        self.video_url = "https://172.20.8.15:6554/renmingdemingyi.mp4"
        ...
        self.body2 = {
            ...
            "callback": self.callback,
            "token": self.token,
            "frame_extract_interval":400,
            "modules":"filter_celebrity:100,"
    ...
    def test_001(self):
        ...
        self.logger.info("now to send  first waverequest,body is  %s!" % self.body )
    ...
    def test_002(self):
        ...
        self.logger.info("now to send second wave request,body is  %s!" % self.body2 )

        # 发送request请求
        r = requests.post(self.url, data=json.dumps(self.body2), headers=self.headers)
    ...
    def test_3(self):
        self.logger.info("self.ids len 1  is %s  len 2 is %s "% (len(self.ids_2[0]),len(self.ids_2[1])))
        if len(self.ids_2[0]) == len(self.ids_2[1]):
            self.logger.error("modules filter not valid !")
            assert False
