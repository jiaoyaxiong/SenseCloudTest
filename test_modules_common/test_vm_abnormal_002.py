...
class test_vm_abnormal_002(common.sensemediaTestBase):

    def __init__(self):
        super(test_vm_abnormal_002, self).__init__("test_vm_abnormal_002")
    ...
    def test_001(self):
        ...
        if r.json().get("error_msg") != "Module interval is not legal: module is filter_celebrity":
            self.logger.error("error_msg should be Module interval is not legal: module is filter_celebrity ,but is %s " % r.json().get("error_msg"))
            assert False

        if r.json().get("http_code") != 400:
            assert False

        if r.json().get("error_code") != 1010114:
            assert False
