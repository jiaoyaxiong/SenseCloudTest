...
class test_vd_abnormal_003(common.sensemediaTestBase):

    def __init__(self):
        super(test_vd_abnormal_003, self).__init__("test_vd_abnormal_003")
    ...
    def test_001(self):
        ...
        if r.json().get("error_msg") != "Part name is not exists: name is null":
            assert False

        if r.json().get("http_code") != 400:
            assert False

        if r.json().get("error_code") != 1010113:
            assert False
