...
"""
用例名字：test_vt_abnormal_006
测试目的：files--MP4-普通电影
预置条件：1，测试桩已启动（接收callback 的post 请求）
         2,私有云已启动
         3,视频文件:  已经放到当前目录
测试步骤：1，构造请求发送files--MP4-普通电影视频给私有云分析
         2，检查响应,记录响应结果
预期结果：1，响应状态码200
         2，应答结果检查：
...
class test_vt_abnormal_006(common.sensemediaTestBase):

    def __init__(self):
        super(test_vt_abnormal_006, self).__init__("test_vt_abnormal_006")
        ...
        # request url
        self.file =("renmingdemingyi.mp4", open('/codes/sensecloud/test_video_transports_common/renmingdemingyi.mp4', 'rb'), "video/mpeg4")
        # TODO
    ...
    #         assert False
    #
    #     if resp_message != "Incorrect request parameters: file callback or token is null":
