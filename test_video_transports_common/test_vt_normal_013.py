...
"""
用例名字：test_vt_normal_013
测试目的：rtsp--MP4-普通电影
预置条件：1，测试桩已启动（接收callback 的post 请求）
         2,私有云已启动
         3,视频文件:  已经放到当前目录
测试步骤：1，构造请求发送rtsp--MP4-普通电影视频给私有云分析
         2，检查响应,记录响应结果
预期结果：1，响应状态码200
         2，应答结果检查：
...
@attr(runtype="normal")
@attr(videotype="other")
class test_vt_normal_013(common.sensemediaTestBase):

    def __init__(self):
        super(test_vt_normal_013, self).__init__("test_vt_normal_013")
        ...
        self.video_url2 = "http://172.20.8.15:90/video_1.mp4"

        self.stream = "    rtsp://172.20.8.15:8081/123.ts"
    ...
    def test_001(self):
        ...
        self.logger.info("now to send request,body2 is  %s!" % self.body)

        # 发送request请求
        r = requests.post(self.url, data=json.dumps(self.body), headers=self.headers)
        r2 = requests.post(self.url, data=json.dumps(self.body2), headers=self.headers)


        self.logger.info(r.text)
        self.logger.info(r2.text)

        self.logger.info("please manul make sure if pass !")
    ...
    #             assert False
    #
    #         statu = r.json().get("content")[0].get("status")
    #         if statu != "RUNNING":
    #             self.logger.info("task must be RUNNING ! but is %s  " % statu)
    #             break
    #         else:
    #             self.logger.info("task not done ,status is %s" % statu )
