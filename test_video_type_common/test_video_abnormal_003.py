...
class test_video_abnormal_003(common.sensemediaTestBase):

    def __init__(self):
        super(test_video_abnormal_003, self).__init__("test_video_abnormal_003")
    ...
    def test_001(self):
        ...
        if r.json().get("error_msg") != "File type is not support":
            self.logger.info("error_msg should be File type is not support")
            assert False
