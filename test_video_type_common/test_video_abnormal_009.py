...
class test_video_abnormal_009(common.sensemediaTestBase):

    def __init__(self):
        super(test_video_abnormal_009, self).__init__("test_video_abnormal_009")
        ...
        # TODO
        self.video_url = "http://172.20.8.15:90/bucunzai.mp4"
        self.stream = ""
    ...
    def test_001(self):
        ...
        if r.json().get("error_msg") != "Request url is invalid":
            assert False

        if r.json().get("http_code") != 400:
            assert False

        if r.json().get("error_code") != 1010107:
            assert False
