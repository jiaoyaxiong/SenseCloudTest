...
"""
用例名字：test_video_normal_001
测试目的：测试最小size的MP4视频是否可以分析
预置条件：1，测试桩已启动（接收callback 的post 请求）
         2,私有云已启动
         3,视频文件:  已经放到当前目录
测试步骤：1，构造请求发送最小size 的MP4 视频给私有云分析
         2，检查响应
预期结果：1，响应状态码200
         2，应答结果检查：
         3，callback ...
...
class test_video_normal_001(common.sensemediaTestBase):

    def __init__(self):
        super(test_video_normal_001, self).__init__("test_video_normal_001")
