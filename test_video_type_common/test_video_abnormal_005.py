...
class test_video_abnormal_005(common.sensemediaTestBase):

    def __init__(self):
        super(test_video_abnormal_005, self).__init__("test_video_abnormal_005")
        ...
        # request url
        self.file =("renmingdemingyi-error.mp4", open('/codes/sensecloud/test_video_type_common/renmingdemingyi-error.mp4', 'rb'), "video/mpeg4")
        # TODO
    ...
    def test_001(self):
        ...
        if r.json().get("error_msg") != "File type is not support":
            assert False

        if r.json().get("http_code") != 400:
            assert False

        if r.json().get("error_code") != 1010103:
            assert False
