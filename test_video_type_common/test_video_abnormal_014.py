...
class test_video_abnormal_014(common.sensemediaTestBase):

    def __init__(self):
        super(test_video_abnormal_014, self).__init__("test_video_abnormal_014")
    ...
    def test_001(self):
        ...
        if r.json().get("error_msg") != "Request stream is invalid":
            self.logger.error("error_msg is %s,not as expected" % r.json().get("error_msg"))
            assert False
