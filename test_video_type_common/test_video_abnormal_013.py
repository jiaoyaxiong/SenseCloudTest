...
class test_video_abnormal_013(common.sensemediaTestBase):

    def __init__(self):
        super(test_video_abnormal_013, self).__init__("test_video_abnormal_013")
    ...
    def test_001(self):
        ...
        if r.json().get("error_msg") != "Not support urls":
            assert False

        if r.json().get("http_code") != 400:
            assert False

        if r.json().get("error_code") != 1010101:
            assert False
