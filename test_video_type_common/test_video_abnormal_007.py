...
class test_video_abnormal_007(common.sensemediaTestBase):

    def __init__(self):
        super(test_video_abnormal_007, self).__init__("test_video_abnormal_007")
        ...
        # request url
        self.file =("renmingdemingyi-space.mp4", open('/codes/sensecloud/test_video_type_common/renmingdemingyi-space.mp4', 'rb'), "video/mpeg4")
        # TODO
    ...
    def test_001(self):
        ...
        if r.json().get("error_msg") != "File is null":
            self.logger.info("error_msg should be File is null")
            assert False

        if r.json().get("http_code") != 400:
            assert False

        if r.json().get("error_code") != 1010112:
            assert False
